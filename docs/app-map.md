...
| Cargo | `cargo/*` (flotte, dossier à onglets, suivi, carte, coût à quai) | `useCargo` | `add_cargo_shipment`, `request_cargo_lookup`, `request_cargo_sync`, `cargo_set_*` ; `cargo_*` (6 tables) ; edge `cargo-lookup`, `cargo-sync` (Maersk + CMA CGM via `_shared/maersk.ts` / `cmacgm.ts`) |
