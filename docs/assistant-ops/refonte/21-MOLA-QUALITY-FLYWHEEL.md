...
## Slice 4 — Profondeur métier + radar partagé + digest auto (PR séparée)

> Cette tranche est livrée dans une **PR distincte** (« Mola : profondeur + radar + digest »)
> car elle embarque une **migration SQL**. Documentée ici pour la vue d'ensemble de la boucle.

Rebasé sur `main` (qui avait ajouté `get_pending_summary` et ouvert la lecture à tout
admin). Adaptations :

- `playbook.ts` : bloc CONNAISSANCE + JUGEMENT métier (pourquoi Bonzini, économie du
  spread USDT→CNY volume≠marge, réflexe de proactivité, signaux normal/alarmant, ton DO),
  composé dans `buildSystemPrompt`.
- `get_pending_summary` (Edge) **enrichi** : au lieu d'un simple comptage, il appelle la
  RPC `mola_operations_radar` (noms, montants, ancienneté, soldes dormants, taux perso).
  Repli automatique sur le comptage si la RPC n'est pas encore déployée → jamais cassé.
- Migration `20260607120000_mola_operations_radar.sql` :
  - RPC `mola_operations_radar(...)` LECTURE SEULE, étiquetée `@mola` (source de vérité
    partagée Mola + cron) ;
  - `run_mola_daily_digest()` + cron `mola-daily-digest` (06:00 UTC) → résumé Telegram
    quotidien. **Inerte** tant que les secrets Vault `telegram_bot_token` /
    `telegram_chat_id` ne sont pas posés (même garde-fou que `run_email_drainer`).

Après push de la migration : lancer `/gen-types` (hygiène ; la RPC read n'est pas dans la
parité des écritures, donc rien ne casse en son absence).
