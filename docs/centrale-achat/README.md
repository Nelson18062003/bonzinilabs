...
| 6 | **Implémentation** (après GO explicite), par lots | [`06-implementation.md`](./06-implementation.md) | 🔄 **En cours** — Backend ✅ (20 RPC `@mola`) · UI : control tower, reste-à-payer, missions (liste/création/rapport), fournisseurs (liste/édition/360), **chaîne de saisie complète mission→fournisseur→commande→ligne→paiement** ✅ ; tout vérifié (type-check+build+118 tests). **rapport PDF + partage natif + photo-preuve** ✅. Module fonctionnellement complet. Reste (post-déploiement) : dictée Mola (test), lien rail Cas 2, outils Mola dédiés + eval. **6 migrations non déployées** (1 grosse migration plus tard) |
