...
| 6 | **Implémentation** (après GO explicite), par lots | [`06-implementation.md`](./06-implementation.md) | 🔄 **En cours** — Backend ✅ (18 RPC `@mola`) · UI : control tower, reste-à-payer, **missions (liste + création + rapport à l'écran)** ✅ ; tout vérifié (type-check+build+118 tests). Reste : PDF, fiches fournisseur/commande, formulaires de saisie, dictée. **6 migrations non déployées** (1 grosse migration plus tard) |
