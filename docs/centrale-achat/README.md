...
| 2 | **Modèle conceptuel & entités** (conçu AI-first) : 10 entités `proc_*`, master-data partagée vs par-client, modèle d'argent Cas 3 (attestation + lien rail), commission double-mode, append-only, RLS, coûts | [`02-modele-donnees.md`](./02-modele-donnees.md) | ⏳ **Rendu — en attente de validation** (6 questions ouvertes) |
