...
| 6 | **Implémentation** (après GO explicite), par lots | [`06-implementation.md`](./06-implementation.md) | 🔄 **En cours** — Backend ✅ (17 RPC `@mola` : 13 écriture + 4 lecture) · UI ✅ démarrée (couche d'accès typée + hooks + control tower + reste-à-payer + routes + menu) ; tout vérifié (type-check+build+118 tests). **6 migrations non déployées** (1 grosse migration prévue plus tard) |
