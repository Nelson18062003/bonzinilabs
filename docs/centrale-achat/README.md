...
| 4 | **Parcours & UX terrain** (révisé) : saisie formulaire/dictée + photos-preuves, control tower, mission/fournisseur/PO 360, rapport PDF, in-app only, wireframes | [`04-parcours.md`](./04-parcours.md) | ✅ **Validé** (saisie formulaires+dictée, photo optionnelle) |
| 5 | **Plan d'implémentation par lots** : 7 lots (Lot 0 fondations → Lot 1 saisie + **catch-up mai 2026** → consultation → rapport → QC → RAG/eval → sécurité), estimations ~80-120 h, critères « fait » | [`05-plan-implementation.md`](./05-plan-implementation.md) | ⏳ **Rendu — en attente de GO Phase 6** |
| 6 | **Implémentation** (après GO explicite), par lots | [`06-implementation.md`](./06-implementation.md) | 🔄 **En cours** — Backend ✅ (21 RPC `@mola`) · UI : control tower, reste-à-payer, missions (liste/création/rapport), fournisseurs (liste/édition/360), **chaîne de saisie complète mission→fournisseur→commande→ligne→paiement** ✅ ; tout vérifié (type-check+build+118 tests). **rapport PDF + partage natif + photo-preuve + lien rail (Cas 2)** ✅. Revues sécurité+qualité passées, `ReportPoCard`, **128 tests**, **intégration Mola** (multi-rôle + résolution `BZ-*` + perms, build-ahead). Reste (post-déploiement) : test Mola, outils Mola dédiés (option) + eval. **5 migrations + edge function `admin-assistant` non déployées** |
| 7 | **Runbook de déploiement** : migrations → gen-types → rôle père → smoke test → catch-up mai 2026 | [`07-deploiement.md`](./07-deploiement.md) | ✅ **Prêt** (à exécuter au déploiement) |
