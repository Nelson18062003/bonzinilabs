...
// ────────────────────────────────────────────────────────────────────────────
// helpers.ts intégré ici.
//
// Le dépôt garde deux fichiers (les helpers sont testés à part, hors Deno :
// src/tests/lib/passkeyHelpers.test.ts). Cette version fondue en un seul
// fichier existe pour le déploiement depuis l'éditeur du tableau de bord
// Supabase, où un import relatif « ./helpers.ts » n'a rien à résoudre.
//
// NE PAS MODIFIER CE FICHIER À LA MAIN : il est régénéré à partir des deux
// sources. Toute correction se fait dans supabase/functions/passkey/.
// ────────────────────────────────────────────────────────────────────────────
 ...
 */
function rpIdForOrigin(origin: string, productionRpId: string): string {
  try {
 ...
 */
function isCounterAcceptable(storedCounter: number, newCounter: number): boolean {
...
function isAllowedOrigin(origin: string | null, allowed: string[]): origin is string {
