...
// Renders the Bonzini rate flyer server-side using Satori (JSX → SVG).
// Returns a self-contained SVG where all text is already converted to vector
// paths, so the client only needs a fast native <img>+canvas step — no font
// loading, no DOM cloning, no memory spike on mobile.
...
// ── Minimal JSX helper (no React needed) ───────────────────────────────────
...
async function getFonts(): Promise<FontDef[]> {
  ...
  const [s800, d400, d600, d700, d800, d900, n400, n700] = buffers;
  fontCache = [
    { name: "Syne",        data: s800, weight: 800, style: "normal" },
...
// ── Flyer element builder ──────────────────────────────────────────────────
interface Rates { alipay: number; wechat: number; bank: number; cash: number }

function buildElement(rates: Rates, dark: boolean): El {
  const now = new Date();
  ...
  const rateCards = [
    { method: "Alipay",        sub: "支付宝",   type: "alipay", rate: rates.alipay },
    { method: "WeChat Pay",    sub: "微信支付", type: "wechat", rate: rates.wechat },
    { method: "Bank Transfer", sub: "银行转账", type: "bank",   rate: rates.bank   },
    { method: "Cash",          sub: "现金",     type: "cash",   rate: rates.cash   },
  ];

  function methodIcon(type: string): El {
    const size = 42, r = size / 2;
    const base = { width: size, height: size, borderRadius: 11, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" };
    if (type === "alipay") return h("div", { style: { ...base, background: "linear-gradient(135deg,#1677FF,#0958d9)" } },
      h("span", { style: { fontFamily: "Noto Sans SC", fontSize: r * 0.9, fontWeight: 700, color: "#fff" } }, "支"));
    if (type === "wechat") return h("div", { style: { ...base, background: "linear-gradient(135deg,#07C160,#06ae56)" } },
      h("span", { style: { fontFamily: "Noto Sans SC", fontSize: r * 0.9, fontWeight: 700, color: "#fff" } }, "微"));
    if (type === "bank") return h("div", { style: { ...base, background: "linear-gradient(135deg,#7c3aed,#5b21b6)" } },
      // Landmark icon (lucide) as inline SVG
      h("svg", { width: r * 0.9, height: r * 0.9, viewBox: "0 0 24 24", fill: "none", stroke: "white", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" },
    ...
    h("div", { style: { width: 440, borderRadius: 28, overflow: "hidden", background: c.bg, position: "relative", boxShadow: c.shadow, display: "flex", flexDirection: "column" } },
      ...
      h("div", { style: { position: "relative", zIndex: 1, padding: "28px 28px 22px", display: "flex", flexDirection: "column" } },
        ...
        h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 } },
          h("div", { style: { display: "flex", alignItems: "center", gap: 10 } },
            ...
            h("div", { style: { display: "flex", flexDirection: "column" } },
              h("div", { style: { fontFamily: "Syne", fontSize: 20, fontWeight: 800, color: c.text } }, "Bonzini"),
              h("div", { style: { fontFamily: "DM Sans", fontSize: 8, fontWeight: 600, color: G, letterSpacing: 2 } }, "DAILY RATE · 每日汇率"),
            ),
          ),
          h("div", { style: { display: "flex", flexDirection: "column", alignItems: "flex-end" } },
            h("div", { style: { fontFamily: "DM Sans", fontSize: 11, fontWeight: 700, color: c.sub } }, `${enD}, ${gz.getDate()} ${enM} ${gz.getFullYear()}`),
            h("div", { style: { fontFamily: "Noto Sans SC", fontSize: 10, color: c.dim, marginTop: 2 } }, `${gz.getFullYear()}年${gz.getMonth() + 1}月${gz.getDate()}日 ${cnD}`),
          ),
        ),

        // TIME
        h("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 14, paddingTop: 6, paddingBottom: 6 } },
          h("span", { style: { fontFamily: "Noto Sans SC", fontSize: 10, fontWeight: 600, color: c.dim, letterSpacing: 1 } }, "广州"),
          h("div", { style: { display: "flex", alignItems: "baseline", fontFamily: "DM Sans", fontSize: 22, fontWeight: 900, color: c.text, letterSpacing: -1 } },
            h("span", null, hh),
            h("span", { style: { color: G } }, ":"),
            h("span", null, mm),
          ),
          h("span", { style: { fontFamily: "Noto Sans SC", fontSize: 12, color: c.dim } }, cnP),
          h("span", { style: { fontFamily: "DM Sans", fontSize: 9, color: c.faint } }, "GMT+8"),
        ),

        divider(
          dark
            ? `linear-gradient(90deg,transparent,${V}20,${G}20,transparent)`
            : `linear-gradient(90deg,transparent,${V}12,${G}12,transparent)`,
          14,
        ),

        // RATE FOR
        h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 12 } },
          h("div", { style: { fontFamily: "DM Sans", fontSize: 11, fontWeight: 600, color: c.dim, letterSpacing: 1 } }, "Exchange rate for · 汇率基于"),
          h("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "center", gap: 8, marginTop: 4 } },
            h("span", { style: { fontFamily: "Syne", fontSize: 34, fontWeight: 800, color: G } }, "1 000 000"),
            h("span", { style: { fontFamily: "DM Sans", fontSize: 15, fontWeight: 800, color: c.dim } }, "XAF"),
          ),
        ),
        ...
        h("div", { style: { display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 } },
          ...rateCards.map((r) =>
            h("div", { style: { display: "flex", alignItems: "center", padding: "14px 16px", borderRadius: 16, background: c.cardBg, border: `1px solid ${c.cardBd}` } },
              methodIcon(r.type),
              h("div", { style: { flex: 1, marginLeft: 12, display: "flex", flexDirection: "column" } },
                h("div", { style: { fontFamily: "DM Sans", fontSize: 13, fontWeight: 700, color: dark ? "rgba(255,255,255,0.75)" : c.text } }, r.method),
                h("div", { style: { fontFamily: "Noto Sans SC", fontSize: 10, color: c.dim, marginTop: 1 } }, r.sub),
              ),
              h("div", { style: { display: "flex", alignItems: "baseline", gap: 3 } },
                h("span", { style: { fontFamily: "DM Sans", fontSize: 16, fontWeight: 700, color: c.dim } }, "¥"),
                h("span", { style: { fontFamily: "DM Sans", fontSize: 50, fontWeight: 900, color: c.text, letterSpacing: 2 } }, fmt(r.rate)),
              ),
            )
          ),
        ),
        ...
        // INSTANT NOTICE
        h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 10, padding: "10px 16px", borderRadius: 10, background: dark ? `linear-gradient(135deg,${V}08,${G}05)` : `linear-gradient(135deg,${V}06,${G}04)`, border: `1px solid ${dark ? `${V}12` : `${V}10`}` } },
          h("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: 4, fontFamily: "DM Sans", fontSize: 11, fontWeight: 600, color: c.sub } },
            h("span", null, "Payments via Alipay, WeChat Pay & Bank Transfer are"),
            h("span", { style: { color: "#34d399", fontWeight: 800 } }, "instant"),
          ),
          h("div", { style: { display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", fontFamily: "Noto Sans SC", fontSize: 10, color: c.dim, marginTop: 3 } },
            h("span", null, "通过支付宝、微信支付和银行转账"),
            h("span", { style: { color: "#34d399" } }, "即时到账"),
          ),
        ),

        // WEBSITE
        h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 10, padding: "14px 20px", borderRadius: 14, background: dark ? `linear-gradient(135deg,${V}15,${G}10)` : `linear-gradient(135deg,${V}10,${G}06)`, border: `1px solid ${dark ? `${V}20` : `${V}15`}` } },
          h("div", { style: { fontFamily: "Syne", fontSize: 22, fontWeight: 800, color: c.text } }, "bonzinilabs.com"),
        ),

        // DISCLAIMER
        h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 10, padding: "8px 12px", borderRadius: 8, background: dark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)" } },
          h("div", { style: { fontFamily: "DM Sans", fontSize: 9, fontWeight: 500, color: c.faint, lineHeight: 1.5, textAlign: "center" } },
          ...
          h("div", { style: { fontFamily: "Noto Sans SC", fontSize: 8, color: c.faint, marginTop: 3, lineHeight: 1.5, textAlign: "center" } },
            "汇率仅供参考，可能因付款方式、交易量和市场状况而异。最终汇率以交易时确认为准。",
          ),
        ),

        divider(dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)", 10),

        // CONTACTS
        h("div", { style: { display: "flex", justifyContent: "center", gap: 24 } },
          h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center" } },
            h("div", { style: { fontFamily: "DM Sans", fontSize: 8, fontWeight: 700, color: c.faint, letterSpacing: 2, marginBottom: 4 } }, "WHATSAPP"),
            h("div", { style: { fontFamily: "DM Sans", fontSize: 14, fontWeight: 800, color: c.sub } }, "+237 652 236 856"),
          ),
          h("div", { style: { width: 1, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)" } }),
          h("div", { style: { display: "flex", flexDirection: "column", alignItems: "center" } },
            h("div", { style: { fontFamily: "DM Sans", fontSize: 8, fontWeight: 700, color: c.faint, letterSpacing: 2, marginBottom: 4 } }, "CHINA · 中国"),
            h("div", { style: { fontFamily: "DM Sans", fontSize: 14, fontWeight: 800, color: c.sub } }, "+86 131 3849 5598"),
          ),
        ),
...
serve(async (req) => {
  ...
  try {
    const body = await req.json() as { rates: Rates; dark?: boolean };
    const { rates, dark = true } = body;
    ...
    const element = buildElement(rates, dark);

    const svg = await satori(element, {
      width: 440,
      height: 870, // conservative: content ≈ 820px + extra background padding
      fonts,
    });
