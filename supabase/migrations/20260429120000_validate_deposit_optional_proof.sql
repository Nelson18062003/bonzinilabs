-- ============================================================
-- Make proof of payment optional for admin deposit validation.
--
-- Avant : la RPC validate_deposit refusait toute validation
-- lorsque deposit_proofs ne contenait aucune ligne pour le dépôt.
--
-- Maintenant : l'admin peut valider sans preuve. La présence ou
-- l'absence de preuve est désormais tracée dans
-- admin_audit_logs.details (clé `had_proofs_at_validation`) pour
-- préserver l'audit trail. Les autres garde-fous (admin only,
-- statut non terminal, lock de wallet) restent inchangés.
-- ============================================================
...
BEGIN
  ...
  -- Audit log — capture proof presence for compliance review
