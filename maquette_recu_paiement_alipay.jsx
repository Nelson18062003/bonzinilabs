...
const B = {
  violet: "#a64af7", violetDark: "#1a1028", violetLight: "#f3ecf8",
  gold: "#f3a745", goldLight: "#fdf4e3",
  orange: "#fe560d", orangeLight: "#fdeee8",
  green: "#10b981", greenLight: "#ecfdf5",
  blue: "#3b82f6", blueLight: "#eff6ff",
  text: "#2d2040", muted: "#7a7290", light: "#f8f6fa",
  border: "#ebe6f0", alipay: "#1677ff", wechat: "#07c160",
};

function fmt(n) { return n.toLocaleString("fr-FR"); }

// ─── LOGO SVG BONZINI (original tracé — NE PAS MODIFIER) ───
...
function ReceiptLayout({ type, reference, children }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 2px 20px rgba(0,0,0,0.08)", border: "1px solid " + B.border,
    }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800;900&display=swap" rel="stylesheet" />
      <div style={{
        background: "linear-gradient(135deg, " + B.violetDark + ", #2a1845)",
        padding: "20px 24px 16px", color: "#fff",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Logo size={32} />
            <span style={{ fontSize: 16, fontWeight: 900, letterSpacing: "-0.3px" }}>Bonzini</span>
          </div>
          <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.5)", fontFamily: "monospace" }}>{reference}</div>
        </div>
        <div style={{ display: "flex", margin: "14px 0 10px", borderRadius: 2, overflow: "hidden", height: 2 }}>
          <div style={{ flex: 2, background: B.gold }} />
          <div style={{ flex: 3, background: B.violet }} />
          <div style={{ flex: 2, background: B.orange }} />
        </div>
        <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: 1.5, textTransform: "uppercase" }}>{typeLabel}</div>
      </div>
      <div style={{ padding: "20px 24px 24px" }}>{children}</div>
      <div style={{
        padding: "12px 24px 16px", borderTop: "1px solid " + B.border,
        display: "flex", justifyContent: "space-between", alignItems: "center",
      }}>
        <div style={{ fontSize: 8, color: B.muted }}>Document généré automatiquement par Bonzini</div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <Logo size={14} />
          <span style={{ fontSize: 8, fontWeight: 700, color: B.muted }}>bonzinilabs.com</span>
        </div>
      </div>
    </div>
  );
...
function InfoRow({ label, value, bold, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 11, color: B.muted, fontWeight: 500 }}>{label}</span>
      <span style={{ fontSize: 12, fontWeight: bold ? 700 : 600, color: color || B.text }}>{value}</span>
    </div>
  );
}
...
function SectionTitle({ children }) {
  return (
    <div style={{
      fontSize: 10, fontWeight: 800, color: B.gold, textTransform: "uppercase",
      letterSpacing: 2, margin: "18px 0 8px", paddingTop: 4,
    }}>{children}</div>
  );
}

function AmountBlock({ amount, details }) {
  return (
    <div style={{
      textAlign: "center", padding: "20px 16px",
      background: B.light, borderRadius: 12,
      border: "1px solid " + B.border, marginBottom: 16,
    }}>
      <div style={{ fontSize: 32, fontWeight: 900, color: B.text, letterSpacing: "-1px" }}>
        {fmt(amount)} <span style={{ fontSize: 14, fontWeight: 600, color: B.muted }}>XAF</span>
      </div>
      {details && (
        <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 10 }}>
          {details.map(d => (
            <div key={d.label}>
              <div style={{ fontSize: 9, color: B.muted, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>{d.label}</div>
              <div style={{ fontSize: 14, fontWeight: 800, color: B.violet, marginTop: 2 }}>{d.value}</div>
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  return (
    <div style={{ minHeight: "100vh", background: "#f0edf3", padding: "24px 16px", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: B.muted, textTransform: "uppercase", letterSpacing: 2 }}>Maquette</div>
        <div style={{ fontSize: 18, fontWeight: 900, color: B.violetDark, marginTop: 4 }}>Reçu de paiement — Alipay</div>
      </div>

      <ReceiptLayout type="paiement" reference="BZ-PY-2026-0020">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <StatusBadge status="Effectué" color={B.violet} bg={B.violetLight} />
          <span style={{ fontSize: 11, color: B.muted }}>07 mars 2026 à 15:12</span>
        </div>

        <AmountBlock amount={700000} details={[
          { label: "Montant envoyé", value: "¥8 071" },
          { label: "Taux appliqué", value: "11 530" + " XAF/M" },
        ]} />

        <SectionTitle>Transaction</SectionTitle>
        <InfoRow label="Mode de paiement" value="Alipay" color={B.alipay} bold />
        <InfoRow label="Date de création" value="07 mars 2026 à 09:43" />
        <InfoRow label="Date de traitement" value="07 mars 2026 à 15:12" />

        <SectionTitle>Client</SectionTitle>
        <InfoRow label="Nom" value="Stephan Tchamdjou" bold />
        <InfoRow label="Téléphone" value="+237 678 148 981" />

        <SectionTitle>Bénéficiaire</SectionTitle>
        <InfoRow label="Nom" value="Bomarkaise" bold />
        <InfoRow label="Identifiant Alipay" value="bomarkaise(*STEPHAN)" />

        {/* QR Code */}
        <div style={{
          marginTop: 16, padding: 20, textAlign: "center",
          background: B.light, borderRadius: 12, border: "1px solid " + B.border,
        }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: B.muted, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 10 }}>QR Code Bénéficiaire</div>
          <div style={{
            width: 140, height: 140, margin: "0 auto",
            background: "#fff", borderRadius: 8, border: "2px solid " + B.border,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <div style={{ fontSize: 10, color: "#ccc", textAlign: "center" }}>[ QR Code<br/>Alipay ]</div>
          </div>
          <div style={{ fontSize: 10, color: B.muted, marginTop: 8 }}>Bomarkaise · Alipay · ¥8 071</div>
        </div>

        {/* Preuve */}
        <SectionTitle>Preuve de paiement</SectionTitle>
        <div style={{ padding: 16, background: B.light, borderRadius: 12, border: "1px solid " + B.border }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: B.muted }}>Capture de confirmation</span>
            <span style={{ fontSize: 10, color: B.muted }}>07 mars 2026 à 15:12</span>
          </div>
          <div style={{
            width: "100%", height: 200, background: "#e8e4ed",
            borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
            border: "1px solid " + B.border,
          }}>
            <div style={{ textAlign: "center", color: "#aaa", fontSize: 11 }}>
              [ Capture d\'écran Alipay ]<br/><span style={{ fontSize: 9 }}>-8,071.00 · 交易成功</span>
            </div>
          </div>
        </div>
      </ReceiptLayout>
