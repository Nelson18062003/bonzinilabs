/**
 * DEV-ONLY maquettes v6 (révision finale d'après retours).
 * FICHE : « Télécharger le reçu » TOUT EN HAUT (sous la réf, avant le montant) ;
 *   taux au format « 1 000 000 XAF = 11 350 ¥ » (XAF d'abord), bien visible ;
 *   preuve = image cliquable (prévisualisation) ; QR bénéficiaire cliquable.
 * LISTE : pastille « À compléter » courte + libellé « Coordonnées manquantes »
 *   sous le nom (rouge), barres d'avancement rouge/lilas/vert.
 * Harness: ?screen=cpay-list-v6 | cpay-detail-v6
 */
import { LOGO_PATH } from '@/mobile/designKit/methods';
import { SURFACE, TEXT, PRIMARY_PILL } from '@/mobile/designKit/tokens';
import { Landmark, Plus, ArrowLeft, ArrowRight, Copy, ChevronRight, Download, QrCode, Check } from 'lucide-react';
    ...
    <div className={cn('mx-auto min-h-screen max-w-[420px]', SURFACE.canvas)}>
      <div className="space-y-6 p-4 pt-6">
        ...
        <button className={cn('flex w-full items-center gap-4 rounded-[24px] p-5 text-left', SURFACE.card, SURFACE.shadow)}>
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#1C1B22] dark:bg-[#F2F1F7]"><Plus className="h-7 w-7 text-white dark:text-[#1B1A24]" strokeWidth={2.4} /></div>
  ...
  return (
    <div className={cn('mx-auto min-h-screen max-w-[420px]', SURFACE.canvas)}>
      ...
      <div className="space-y-4 p-4 pt-1">
        ...
        {/* BÉNÉFICIAIRE — nuancé Alipay, QR cliquable */}
        ...
        {/* PREUVE — image cliquable */}
        <button className={cn('flex w-full items-center gap-3 rounded-[22px] p-5 text-left', SURFACE.card, SURFACE.shadow)}>
          <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#ECE8F6] dark:bg-[#2A2738]">
            <div className="h-full w-full bg-[linear-gradient(135deg,#D8D2EC,#C3BDD2)] dark:bg-[linear-gradient(135deg,#3A3550,#2A2738)]" />
          </div>
          <div className="min-w-0 flex-1">
