...
// ─── TYPES (exported so generateClientStatement.ts can re-export them) ─────
...
// Abbreviate long type labels so they don't truncate in the Type column
...
const TYPE_COLOR: Record<string, string> = {
  'Dépôt':         C.green,
  'Paiement':      C.orange,
  'Remboursement': '#3b82f6',
  'Ajustement':    C.muted,
};

const TYPE_BG: Record<string, string> = {
  'Dépôt':         C.greenLight,
  'Paiement':      C.orangeLight,
  'Remboursement': '#eff6ff',
  'Ajustement':    '#f3f4f6',
};
...
/**
 * Format a number with normal spaces (U+0020) as thousand separators.
 * Avoids non-breaking spaces (U+00A0) which cause "1 /732 /800" in some PDF engines.
 */
...
/**
 * Sanitize text for PDF output. Replaces Unicode characters that may
 * corrupt in certain PDF font implementations (arrows, dashes, special spaces).
 */
...
const PAD = 22; // page padding in pt

// Column widths (pt). "motif" column gets flex: 1 (remaining space).
// A4 landscape content width ≈ 841.89 - 2*22 = 797.89pt
const COL = {
  date:   62,
  ref:    70,
  type:   72,  // wide enough for "Rembours." badge
  debit:  88,
  credit: 88,
  solde:  90,
};
...
const s = StyleSheet.create({
  page: {
    padding: PAD,
    paddingBottom: PAD + 20, // extra bottom padding for footer
    fontFamily: 'DM Sans',
    fontSize: 9,
    color: C.text,
  ...
  headerStripeGold:   { flex: 2, backgroundColor: C.gold },
  headerStripeViolet: { flex: 3, backgroundColor: C.violet },
  headerStripeOrange: { flex: 2, backgroundColor: C.orange },
  headerInfo: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
...
// ─── LOGO SVG — identical paths to PDFHeader.tsx — NE PAS MODIFIER ──────────
function PdfLogo({ size = 36 }: { size?: number }) {
  return (
...
export function ClientStatementPDF({ data }: { data: StatementInput }) {
  const {
    clientName,
    clientPhone,
    clientEmail,
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        {/* ═══ BRAND HEADER — fixed, repeats on every page ═══ */}
        <View style={s.header} fixed>
          {/* Top row: logo + brand name | document box */}
          <View style={s.headerTop}>
            <View>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <PdfLogo size={36} />
                <View style={{ marginLeft: 10 }}>
                  <Text style={s.headerBrandName}>Bonzini</Text>
                  <Text style={s.headerSubtitle}>Paiements CEMAC {'>'} Chine</Text>
                </View>
              </View>
            </View>
            <View style={s.headerDocBox}>
              <Text style={s.headerDocLabel}>Document</Text>
              <Text style={s.headerDocTitle}>Relevé de compte client</Text>
            </View>
          </View>

          {/* Tricolor stripe */}
          <View style={s.headerStripe}>
            <View style={s.headerStripeGold} />
            <View style={s.headerStripeViolet} />
            <View style={s.headerStripeOrange} />
          </View>
          ...
          <View style={s.headerInfo}>
            ...
            <View style={s.headerPeriodRight}>
              <Text style={s.headerGoldLabel}>Période</Text>
              <Text style={s.headerPeriodText}>Du {san(periodFrom)}</Text>
              <Text style={s.headerPeriodText}>Au {san(periodTo)}</Text>
              <Text style={s.headerEmitted}>Émis le {san(generatedAt)}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={s.summaryRow}>
          ...
          <View style={[s.summaryBox, s.summaryBoxBorder, { backgroundColor: C.orangeLight }]}>
            <Text style={s.summaryLabel}>Total paiements</Text>
            <Text style={[s.summaryValue, { color: C.orange }]}>-{fmtNum(totalDebits)}</Text>
            <Text style={s.summarySub}>XAF</Text>
          </View>
        ...
        <View style={s.tableHead}>
          ...
          <Text style={[s.tableHeadCell, { width: COL.ref }]}>Réf.</Text>
          <Text style={[s.tableHeadCell, { width: COL.type }]}>Type</Text>
          <Text style={[s.tableHeadCell, { flex: 1 }]}>Motif / Description</Text>
          <Text style={[s.tableHeadCell, { width: COL.debit, textAlign: 'right' }]}>Débit (XAF)</Text>
          <Text style={[s.tableHeadCell, { width: COL.credit, textAlign: 'right' }]}>Crédit (XAF)</Text>
          <Text style={[s.tableHeadCell, { width: COL.solde, textAlign: 'right' }]}>Solde (XAF)</Text>
        </View>

        {/* ═══ DATA ROWS ═══ */}
        {movements.map((m, i) => {
          const typeLabel = TYPE_LABEL[m.type] ?? m.type;
          const typeColor = TYPE_COLOR[m.type] ?? C.muted;
          const typeBg    = TYPE_BG[m.type]    ?? '#f3f4f6';
          const soldeColor = m.solde < 0 ? C.orange : C.text;
          ...
          return (
            <View
              ...
              wrap={false}
            >
              <Text style={s.cellDate}>{fmtDate(m.date)}</Text>
              <Text style={s.cellRef}>{san(m.reference)}</Text>
              <View style={s.cellTypeWrapper}>
              ...
              <Text style={s.cellCredit}>
                {m.credit > 0 ? `+${fmtNum(m.credit)}` : ''}
              </Text>
              <Text style={[s.cellSolde, { color: soldeColor }]}>
                {fmtNum(m.solde)}
              </Text>
            </View>
          );
        ...
        <View style={s.totalsRow} wrap={false}>
          <Text style={[s.totalsLabel, { width: COL.date }]}> </Text>
          <Text style={[s.totalsLabel, { width: COL.ref }]}> </Text>
          <Text style={[s.totalsLabel, { width: COL.type }]}> </Text>
        ...
        <View style={s.footer} fixed>
          <Text style={s.footerLeft}>
            Document généré automatiquement par Bonzini — {san(generatedAt)}
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            />
            <Text style={[s.footerPageNum, { marginLeft: 10 }]}>·</Text>
            <FooterLogoSmall />
            <Text style={s.footerRightText}>bonzinilabs.com</Text>
          </View>
        </View>
