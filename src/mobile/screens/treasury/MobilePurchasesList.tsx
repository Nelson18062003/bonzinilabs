...
import { Loader2, ArrowDownToLine, Trash2, Ban, ChevronRight, AlertTriangle } from 'lucide-react';
...
export function MobilePurchasesList() {
  ...
  const purchases = (data ?? []).filter((op) => {
    ...
    <div className="flex flex-col min-h-full bg-background">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-3 text-[12px]">
          <span className="text-muted-foreground">Total achats actifs : </span>
          ...
          <span className="font-bold">
            {fmt(
              purchases.filter((p) => !p.voided_at).reduce((s, p) => s + Number((p as { xaf_amount: number }).xaf_amount ?? 0), 0),
              0,
            )}{' '}
            XAF
          </span>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {purchases.map((op) => (
              <PurchaseCard
                key={op.id}
                op={op as Extract<OperationRow, { kind: 'purchase' }>}
