    ...
    <div className={desktop ? '' : 'flex flex-col min-h-full'}>
      {desktop ? (
        <header className="mb-5">
          <h2 className="text-[24px] font-bold tracking-tight text-foreground">Veille macro</h2>
      ...
      <div className={desktop ? 'grid grid-cols-1 items-start gap-4 lg:grid-cols-2' : 'flex-1 px-4 py-4 space-y-4'}>
        {/* Prédiction IA */}
        {prediction && (
          <div className="rounded-lg border-2 border-purple-600/20 bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950/30 dark:to-blue-950/30 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[14px] font-bold text-[#1E1E1E] dark:text-[#F5F5F5]">
                Prédiction IA · 24h
              </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="text-3xl font-bold text-[#1E1E1E] dark:text-[#F5F5F5]">
                {prediction.predicted_rate.toLocaleString('fr-FR')}
              </div>
              <div className={`flex items-center gap-1 text-sm font-bold ${
                prediction.direction === 'up' ? 'text-[#009951]' :
                prediction.direction === 'down' ? 'text-[#C00F0C]' : 'text-muted-foreground'
              }`}>
            ...
            {prediction.reasoning && (
              <div className="text-[14px] text-foreground/80 leading-relaxed mb-3">
                {prediction.reasoning}
              </div>
            )}
            {prediction.action_recommended && (
              <div className="text-[14px] font-semibold bg-[#2C2C2C]/10 dark:bg-purple-400/20 text-[#1E1E1E] dark:text-[#F5F5F5] rounded-lg px-3 py-2">
                🎯 {prediction.action_recommended}
              </div>
            )}
          </div>
          ...
          {macro ? (
            <div className="grid grid-cols-2 gap-2 text-[14px]">
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">🛢 Brent</div>
                <div className="font-bold">{fmt(macro.oil_brent)} $</div>
              </div>
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">💵 DXY</div>
                <div className="font-bold">{fmt(macro.dxy)}</div>
              </div>
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">🇪🇺 EUR/USD</div>
                <div className="font-bold">{fmt(macro.eur_usd, 4)}</div>
              </div>
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">🇨🇳 USD/CNY</div>
                <div className="font-bold">{fmt(macro.cny_usd, 4)}</div>
              </div>
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">₿ BTC</div>
                <div className="font-bold">{macro.btc_usd ? Math.round(macro.btc_usd).toLocaleString('fr-FR') : '—'} $</div>
              </div>
              <div className="bg-muted/50 rounded-lg p-2.5">
                <div className="text-[14px] uppercase text-muted-foreground">🛢 WTI</div>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          {trumpPosts && trumpPosts.length > 0 ? (
            <div className="space-y-2.5">
              {trumpPosts.map((p) => (
                <div key={p.id} className="border-l-2 border-[#C00F0C] pl-3 py-1">
            ...
            <div className="space-y-2">
              {macro.news_headlines.slice(0, 6).map((h, i) => (
                <div key={i} className="text-[14px] leading-relaxed">
                  <span className="text-[14px] text-muted-foreground mr-1.5">[{h.source.replace('Google News (', '').replace(')', '')}]</span>
                  {h.title}
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2.5">
              {briefs.map((b) => (
                <div key={b.id} className="bg-muted/30 rounded-lg p-2.5">
                  ...
                  {!b.telegram_sent && (
                    <div className="flex items-center gap-1 text-[14px] text-[#C00F0C] mb-1">
                      <AlertCircle className="w-3 h-3" />
                      Échec Telegram : {b.telegram_error}
                    </div>
                  )}
                  <div className="text-[14px] text-muted-foreground line-clamp-3 whitespace-pre-line">
