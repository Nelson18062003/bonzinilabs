...
export function DesktopTopbar({ onOpenPalette }: { onOpenPalette: () => void }) {
  ...
  return (
    <header
      style={{ height: LAYOUT.topbar }}
      className={cn(
        'sticky top-0 z-20 flex items-center gap-3 border-b px-6',
        DS.line,
    ...
    >
      ...
      <nav aria-label="Fil d'ariane" className="min-w-0">
        <ol className="flex min-w-0 items-center gap-2">
          ...
          <li aria-hidden className="shrink-0">
            <ChevronRight className={cn('h-3.5 w-3.5', DFG.muted)} />
          </li>
          <li aria-current="page" className={cn(DT.body, DFG.strong, 'truncate font-bold')}>
            {trail.page}
          </li>
        </ol>
      </nav>
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <button
          ...
          className={cn(
            'flex h-8 w-[300px] items-center gap-2 rounded-md border px-3 text-left transition-colors',
            DS.line,
            DS.well,
            DS.hover,
            DFOCUS,
        ...
        >
          <Search className={cn('h-4 w-4 shrink-0', DFG.muted)} />
          <span className={cn(DT.label, DFG.muted, 'flex-1 truncate font-normal')}>Rechercher ou demander à Mola…</span>
          <kbd className={cn('flex h-4 shrink-0 items-center rounded border px-1 text-[12px] font-bold leading-4', DS.line, DS.card, DFG.muted)}>⌘K</kbd>
        </button>
        ...
        {rate?.rate_alipay ? (
          ...
          <div className={cn('hidden h-8 items-center gap-2 rounded-md border px-3 xl:flex', DS.line, DS.card)}>
            <span className="h-2 w-2 rounded-full bg-[#2E7D52] dark:bg-[#7FCBA0]" />
            ...
            <span className={cn(DT.label, DFG.muted, 'hidden font-normal 2xl:inline')}>· {today}</span>
          </div>
        ) : null}
