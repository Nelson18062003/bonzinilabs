  ...
  return (
    <div>
      <SecLabel
        ...
        right={
          canManage && (
            <button type="button" onClick={() => setOpen(true)} className={cn('inline-flex h-7 items-center gap-1 px-2.5 text-[11px] max-lg:text-[14px] font-semibold', SOFT_PILL)}>
      ...
      {!docs || docs.length === 0 ? (
        <p className={cn('text-[12.5px] max-lg:text-[14px]', TEXT.muted)}>Aucun document. Le bill of lading, la facture et le télex ont leur place ici.</p>
      ) : (
          ...
          {docs.map((d) => (
            <li key={d.id} className="flex items-center gap-2.5 py-2">
              ...
              <button type="button" onClick={() => openCargoDocument(d)} className="min-w-0 flex-1 text-left">
                <span className={cn('block truncate text-[12.5px] max-lg:text-[14px] font-semibold', TEXT.strong)}>{DOCUMENT_KIND_LABEL[d.kind] ?? d.kind}</span>
                <span className={cn('block truncate text-[11px] max-lg:text-[14px]', TEXT.muted)}>{d.file_name} · {absShort(d.created_at)}</span>
              </button>
              {canManage && (
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {KINDS.map((k) => (
                <button key={k} type="button" onClick={() => setKind(k)} className={cn('h-8 rounded-md px-3 text-[12px] max-lg:text-[14px] font-semibold', k === kind ? PRIMARY_PILL : SOFT_PILL)}>
          ...
          <div>
            ...
            <button type="button" onClick={() => inputRef.current?.click()} className={cn('flex h-20 w-full items-center justify-center rounded-lg border border-dashed border-input text-[13px] max-lg:text-[14px]', SURFACE.inset, file ? TEXT.strong : TEXT.muted)}>
