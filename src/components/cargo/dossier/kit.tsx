...
export function SectionHead({ title, meta, action }: { title: ReactNode; meta?: ReactNode; action?: ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-black/[0.06] px-5 py-3 dark:border-white/[0.06]">
      <span className={cn('text-[13px] max-lg:text-[16px] font-bold', TEXT.strong)}>{title}</span>
...
export function Fact({ label, value, hint, className }: { label: string; value: ReactNode; hint?: ReactNode; className?: string }) {
  return (
    <div className={cn('min-w-0', className)}>
      <div className={cn('text-[11px] max-lg:text-[16px] font-bold uppercase tracking-wider max-lg:normal-case max-lg:tracking-normal', TEXT.muted)}>{label}</div>
      <div className={cn('mt-1 text-[13px] max-lg:text-[16px] font-semibold leading-snug tabular-nums', TEXT.strong)}>{value}</div>
      {hint != null && <div className={cn('mt-0.5 text-[11.5px] max-lg:text-[16px] font-normal', TEXT.muted)}>{hint}</div>}
    </div>
  );
}
...
export function Empty({ title, children }: { title: string; children?: ReactNode }) {
  return (
    <div className="py-6 text-center">
      <p className={cn('text-[13px] max-lg:text-[16px] font-semibold', TEXT.strong)}>{title}</p>
      {children != null && <p className={cn('mx-auto mt-1 max-w-sm text-[12.5px] max-lg:text-[16px]', TEXT.muted)}>{children}</p>}
    </div>
  );
}
