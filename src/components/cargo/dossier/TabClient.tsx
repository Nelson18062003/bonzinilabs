    ...
    <div className="grid grid-cols-[minmax(0,1fr)_360px] gap-5 max-lg:grid-cols-1">
      <div className="space-y-5">
        <Section
          ...
          action={canManage ? (
            client ? (
              <button type="button" onClick={() => update.mutate({ id: s.id, patch: { client_id: null } })} className={cn('inline-flex h-7 items-center gap-1 px-2.5 text-[11.5px] max-lg:text-[14px] font-semibold', SOFT_PILL)}>
                <Link2Off className="h-3 w-3" /> Détacher
              </button>
            ) : (
              <button type="button" onClick={() => setLinkOpen(true)} className={cn('inline-flex h-7 items-center gap-1 px-2.5 text-[11.5px] max-lg:text-[14px] font-semibold', SOFT_PILL)}>
        ...
        >
          {client ? (
            <>
              ...
              <button type="button" onClick={() => navigate(`/m/clients/${client.id}`)} className={cn('mt-4 inline-flex h-8 items-center px-3 text-[12px] max-lg:text-[14px] font-semibold', SOFT_PILL)}>
                  ...
                  <li key={o.id}>
                    <button type="button" onClick={() => navigate(`/m/cargo/${o.id}`)} className="flex w-full items-center justify-between gap-3 py-2.5 text-left">
                      <span className="flex min-w-0 items-center gap-2">
                        ...
                        <span className={cn('truncate text-[12.5px] max-lg:text-[14px]', TEXT.muted)}>{o.vessel_name ?? '—'}</span>
                      </span>
                      <span className="flex shrink-0 items-center gap-2.5">
                        <span className={cn('text-[12.5px] max-lg:text-[14px] font-semibold tabular-nums', TEXT.strong)}>{o.pod_name} · {fmtDay(bestEta(o).date)}</span>
      ...
      <CenterDialog
        ...
        footer={<button type="button" onClick={() => setLinkOpen(false)} className={cn('ml-auto h-9 px-4 text-[13px] max-lg:text-[14px] font-semibold', SOFT_PILL)}>Fermer</button>}
      >
          ...
          {(options ?? []).map((c) => (
            <li key={c.id}>
              <button type="button" onClick={() => link(c.id)} className={cn('flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left hover:bg-muted/50')}>
                <span className={cn('text-[13px] max-lg:text-[14px] font-semibold', TEXT.strong)}>{c.first_name} {c.last_name}</span>
                <span className={cn('text-[12px] max-lg:text-[14px]', TEXT.muted)}>{c.company_name || '—'}</span>
              </button>
            </li>
          ))}
          {options && options.length === 0 && <li className={cn('px-3 py-6 text-center text-[13px] max-lg:text-[14px]', TEXT.muted)}>Aucun client trouvé.</li>}
        </ul>
      </CenterDialog>
    </div>
  );
