...
import { SURFACE, TEXT, Chip, PrimaryPill, SoftPill, StatusPill } from '@/mobile/designKit';
...
export function CountryRatesCard({ activeRate, adjustments, isLoading, onOpenFlyer, variant = 'mobile', className }: CountryRatesCardProps) {
  ...
  // sous les yeux dans « Taux actifs ».
  ...
  // Brouillon d'écart : `null` = valeur serveur. Changer de pays l'abandonne.
  const [draft, setDraft] = useState<string | null>(null);
  const pick = (key: string) => { setCountryKey(key); setDraft(null); };

  const draftPct = draft === null ? null : parseDecimal(draft);
  const draftValid = draftPct !== null && Number.isFinite(draftPct) && Math.abs(draftPct) <= MAX_ABS_PCT;
                  ...
                  <div className="-mx-1 mt-2 flex items-center gap-2 overflow-x-auto px-1 pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {COUNTRY_ADJUSTMENT_PRESETS.map((preset) => (
                      <Chip
                        ...
                        active={effectivePct === preset && (draft === null || draftValid)}
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <TextField
                        variant="decimal"
                        size="sm"
                        value={draft ?? String(selected.percentage)}
                        onChange={(e) => setDraft(e.target.value)}
                        wrapperClassName={desktop ? 'w-[72px]' : 'w-[84px]'}
                        controlClassName={cn('text-right font-bold tabular-nums', desktop ? 'h-9 text-[13px]' : 'h-11')}
                        aria-label={`Écart ${selected.label} en pourcentage`}
                      />
