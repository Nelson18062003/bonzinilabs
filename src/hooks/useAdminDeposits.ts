...
export function useAdminUploadProofs() {
  ...
  return useMutation({
    mutationFn: async ({ depositId, userId, files, depositStatus }: {
      ...
      // If deposit was awaiting_proof, advance to proof_submitted
...
export function useAdminDeleteProof() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // If no proofs remain and deposit is not in a terminal state, revert to awaiting_proof
      if (count === 0) {
        ...
        if (deposit && !['validated', 'rejected'].includes(deposit.status)) {
          await supabaseAdmin.from('deposits')
