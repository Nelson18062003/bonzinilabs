...
export default function App() {
  ...
  const V = "#A947FE", G = "#F3A745", O = "#FE560D";
  const rates = [
    { method: "Alipay", sub: "支付宝", icon: "支", bg: "#1677ff", rate: 11500 },
    { method: "WeChat Pay", sub: "微信支付", icon: "微", bg: "#07c160", rate: 11500 },
    { method: "Bank Transfer", sub: "银行转账", icon: "🏦", bg: V, rate: 11500 },
    { method: "Cash", sub: "现金", icon: "¥", bg: "#dc2626", rate: 11450 },
  ];
  function fmt(n) { return n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " "); }

  const c = dark ? {
    pageBg: "#050208", bg: "linear-gradient(170deg,#0f0820,#120a22 50%,#0a0514)", text: "#fff", sub: "rgba(255,255,255,0.45)", dim: "rgba(255,255,255,0.25)", faint: "rgba(255,255,255,0.15)", cardBg: "rgba(255,255,255,0.03)", cardBd: "rgba(255,255,255,0.06)", shadow: `0 40px 80px rgba(0,0,0,0.6),0 0 160px ${V}08`,
  } : {
    pageBg: "#f0edf4", bg: "linear-gradient(170deg,#fff,#faf8fc 50%,#f5f2f8)", text: "#1a1028", sub: "rgba(26,16,40,0.5)", dim: "rgba(26,16,40,0.3)", faint: "rgba(26,16,40,0.15)", cardBg: "rgba(26,16,40,0.02)", cardBd: "rgba(26,16,40,0.08)", shadow: `0 20px 60px rgba(0,0,0,0.08),0 0 80px ${V}05`,
  };

  return (
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: c.pageBg, padding: 20, gap: 12 }}>
      ...
      <button onClick={() => setDark(!dark)} style={{ padding: "6px 16px", borderRadius: 20, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)", border: "none", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 700, color: dark ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.4)" }}>{dark ? "Light version" : "Dark version"}</button>

      <div style={{ width: 440, borderRadius: 28, overflow: "hidden", background: c.bg, position: "relative", boxShadow: c.shadow }}>
        {dark && <><div style={{ position: "absolute", top: -80, right: -40, width: 250, height: 250, borderRadius: "50%", background: `radial-gradient(circle,${V}10,transparent 70%)`, pointerEvents: "none" }} /><div style={{ position: "absolute", bottom: 120, left: -60, width: 200, height: 200, borderRadius: "50%", background: `radial-gradient(circle,${G}08,transparent 70%)`, pointerEvents: "none" }} /></>}
        ...
        <div style={{ position: "relative", zIndex: 1, padding: "28px 28px 22px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 14, padding: "6px 0" }}>
            ...
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 900, color: c.text, letterSpacing: "-1px" }}>{h}<span style={{ color: G, animation: "blink 1s infinite" }}>:</span>{m}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            {rates.map((r, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", padding: "14px 16px", borderRadius: 16, background: c.cardBg, border: `1px solid ${c.cardBd}` }}>
                <div style={{ width: 42, height: 42, borderRadius: 11, background: r.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 900, color: "#fff", boxShadow: `0 4px 16px ${r.bg}30`, flexShrink: 0 }}>{r.icon}</div>
          ...
          <div style={{ textAlign: "center", marginBottom: 10, padding: "10px 16px", borderRadius: 10, background: dark ? `linear-gradient(135deg,${V}08,${G}05)` : `linear-gradient(135deg,${V}06,${G}04)`, border: `1px solid ${dark ? `${V}12` : `${V}10`}` }}>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 600, color: c.sub, lineHeight: 1.5 }}>Payments via Alipay, WeChat Pay & Bank Transfer are <span style={{ color: "#34d399", fontWeight: 800 }}>instant</span></div>
            <div style={{ fontFamily: "'Noto Sans SC',sans-serif", fontSize: 10, color: c.dim, marginTop: 3 }}>通过支付宝、微信支付和银行转账<span style={{ color: "#34d399" }}>即时到账</span></div>
          </div>
          ...
          <div style={{ textAlign: "center", marginBottom: 10, padding: "8px 12px", borderRadius: 8, background: dark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)" }}>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, fontWeight: 500, color: c.faint, lineHeight: 1.5 }}>Rates are indicative and may vary based on payment method, transaction volume and market conditions. Final rate confirmed at time of transaction.</div>
            <div style={{ fontFamily: "'Noto Sans SC',sans-serif", fontSize: 8, color: c.faint, marginTop: 3, lineHeight: 1.5 }}>汇率仅供参考，可能因付款方式、交易量和市场状况而异。最终汇率以交易时确认为准。</div>
          </div>
